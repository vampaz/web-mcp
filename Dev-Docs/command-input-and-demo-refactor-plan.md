...
- Phase 5: the React `when` type is now a plain boolean derived during render (React cannot make an arbitrary getter reactive). Runtime keeps a legacy JS getter compatibility path and evaluates it once per render. `docs/react.md` was updated. React and Svelte return the Vue-style `{ unregister, getRegistration }` handle.
